...
module.exports = [
 {
    ...
    operate: async (context) => {
        const { m, mess, text, isCreator, reply,Jinwoo } = context;
        ...
        await Jinwoo.groupParticipantsUpdate(m.chat, [bws], "add");
        reply(mess.done);
    }
 },
  {
...
},
 {
...
},
  {
    command: ['close'],
    operate: async (context) => {
        const { m, mess, isAdmins, isCreator, isBotAdmins, Jinwoo, reply } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
    command: ['closetime'],
    operate: async (context) => {
        const { m, mess, args, isAdmins, isCreator, isBotAdmins, Jinwoo, reply } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
    command: ['delppgroup'],
    operate: async (context) => {
        const { m, mess, isAdmins, isCreator, isBotAdmins, Jinwoo, reply, from } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);
...
}, 
  {
    command: ['demote'],
    operate: async (context) => {
        const { m, mess, text, isAdmins, isGroupOwner, isCreator, isBotAdmins, Jinwoo } = context;
        if (!m.isGroup) return reply(mess.group);
        ...
        let bwstq = m.mentionedJid[0]
            ...
            ? m.quoted.sender
            : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
        await Jinwoo.groupParticipantsUpdate(m.chat, [bwstq], "demote");
        reply(mess.done);
    }
},
 {
    ...
    operate: async (context) => {
        const { m, mess, args, isAdmins, isGroupOwner, isCreator, isBotAdmins, Jinwoo, prefix, command } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
        if (!isBotAdmins) return reply(mess.admin);

        if (args[0] === "on") {
            await Jinwoo.groupSettingUpdate(m.chat, "unlocked").then(
                (res) => reply(`*Successful, members can edit group info*`)
            );
        } else if (args[0] === "off") {
            await Jinwoo.groupSettingUpdate(m.chat, "locked").then((res) =>
...
}, 
 {
  ...
  operate: async ({ Jinwoo, m, reply, isAdmins, isGroupOwner, isCreator, mess, isBotAdmins, groupMetadata }) => {
    if (!m.isGroup) return reply(mess.group);
    if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
    if (!isBotAdmins) return reply(mess.admin);

    let response = await Jinwoo.groupInviteCode(m.chat);
    Jinwoo.sendText(
      m.chat,
...
},
 {
    command: ['hidetag', 'tag'], 
    operate: async (context) => {
        const { m, isAdmins, isGroupOwner, isCreator, mess, participants, Jinwoo, isBotAdmins } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
  {
    ...
    operate: async (context) => {
        ...
        let group = m.chat;
        let link = "https://chat.whatsapp.com/" + (await Jinwoo.groupInviteCode(group));
...
},
 {
  ...
  operate: async (context) => {
    const { m, mess, args, store, botNumber, Jinwoo, reply } = context;
    if (!m.isGroup) return reply(mess.group);
...
},
  {
    command: ['mediatag'],
    operate: async (context) => {
        const { m, isAdmins, mess, participants, Jinwoo, isBotAdmins } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
    command: ['open'],
    operate: async (context) => {
        const { m, mess, isAdmins, isCreator, isBotAdmins, Jinwoo } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
    command: ['opentime'],
    operate: async (context) => {
        const { m, mess, args, isAdmins, isCreator, isBotAdmins, Jinwoo } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isCreator) return reply(mess.notadmin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
    command: ['poll'],
    operate: async (context) => {
        const { m, mess, text, isCreator, prefix, Jinwoo, isGroup } = context;
...
},
  {
    command: ['promote'],
    operate: async (context) => {
        const { m, mess, text, isAdmins, isGroupOwner, isCreator, isBotAdmins, quoted, Jinwoo } = context;
        if (!m.isGroup) return reply(mess.group);
        if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin);
        if (!isBotAdmins) return reply(mess.admin);
...
},
 {
  ...
  {
  command: ['setppgroup'],
  operate: async ({ m, reply, mess, isAdmins, isCreator, isBotAdmins, Jinwoo, quoted, mime, prefix, command, args }) => {
    if (!m.isGroup) return reply(mess.group);
    if (!isAdmins && !isCreator) return reply(mess.notadmin);
    if (!isBotAdmins) return reply(mess.admin);
...
},
{
...
},
 {
  ...
  operate: async ({ Jinwoo, m, reply, mess, participants, isGroupAdmins, isCreator, sleep, groupMetadata }) => {
    if (!m.isGroup) return reply(mess.group);
 ...
 {
  command: ['vcf'],
  operate: async ({ Jinwoo, m, reply, mess, participants, isGroupAdmins, isCreator, groupMetadata }) => {
    if (!m.isGroup) return reply(mess.group);
