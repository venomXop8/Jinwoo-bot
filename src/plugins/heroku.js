const updateHerokuVar = async ({ Jinwoo, m, reply, isCreator, key, value, setHerokuEnvVar }) => {
...
module.exports = [
  {
    command: ["addvar"],
    operate: async ({ Jinwoo, m, reply, isCreator, full_args, setHerokuEnvVar }) => {
      if (!isCreator) return;
  ...
  {
    command: ["delvar"],
    operate: async ({ Jinwoo, m, reply, isCreator, full_args, deleteHerokuEnvVar }) => {
      if (!isCreator) return;
  ...
  {
    command: ["setbotname"],
    operate: async ({ Jinwoo, m, reply, isCreator, text, setHerokuEnvVar }) => {
      if (!text) return reply("*Provide a bot name*\n\nExample: .setbotname Jinwoo Alpha");
      await updateHerokuVar({ Jinwoo, m, reply, isCreator, key: "BOT_NAME", value: text.trim(), setHerokuEnvVar });
    }
  },
    {
    command: ["setname"],
    operate: async ({ Jinwoo, m, reply, isCreator, text, setHerokuEnvVar }) => {
      ...
      await updateHerokuVar({ Jinwoo, m, reply, isCreator, key: "OWNER_NAME", value: text.trim(), setHerokuEnvVar });
    }
  },
    {
    ...
    operate: async ({ Jinwoo, m, reply, isCreator, text, setHerokuEnvVar }) => {
      ...
      await updateHerokuVar({ Jinwoo, m, reply, isCreator, key: "OWNER_NUMBER", value: text.trim(), setHerokuEnvVar });
    }
  },
  {
    command: ['setvar'],
    operate: async (context) => {
        ...
        try {
            ...
            await reply(`*Jinwoo Bot will restart to apply the new environment variable. Please wait a moment!*`);
        } catch (error) {
